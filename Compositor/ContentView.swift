    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            if session.tool == .idle {
                HStack(spacing: 16) {
                    Text("选择一个工具").font(ToolHeaderStyle.titleFont)
                    Spacer()
                }.padding(.horizontal, 18).toolHeaderBar()
                Divider()
            }
    ...
    private var statusBar: some View {
        HStack(spacing: 16) {
            ...
            if session.showsBusy {
                ...
                Text(session.tool == .marquee ? (session.marqueeKind == .ellipse ? "拖动绘制椭圆 · Shift 添加 · Option 减去 · 拖动中再按 Shift 变正圆 · 在内部拖动移动 · Delete 清除 · ⌘D 取消选择" : "拖动绘制矩形 · Shift 添加 · Option 减去 · 拖动中再按 Shift 变正方形 · 在内部拖动移动 · ⌘ 拖动移动像素 · Delete 清除 · ⌘D 取消选择") : session.tool == .wand ? (session.wandMode == .object ? "点击对象选择其轮廓 · Tab 切换魔棒 · Shift 添加 · Option 减去 · 在内部拖动移动 · ⌘ 拖动移动像素 · Delete 清除 · ⌘D 取消选择" : "点击选取相似颜色 · Tab 切换对象 · Shift 添加 · Option 减去 · 在内部拖动移动 · ⌘ 拖动移动像素 · Delete 清除 · ⌘D 取消选择") : session.tool == .lasso ? (session.lassoKind == .freehand ? "拖动建立选区 · 在内部拖动移动 · Shift 添加 · Option 减去 · Delete 清除 · ⌥⌫/⌘⌫ 填充 · ⌘D 取消选择" : "点击各角点 · 点击起点/双击/回车闭合 · Delete 删除角点 · Escape 取消") : session.tool == .brush ? (session.brushMode == .erase ? "拖动擦除" : "拖动绘制") + " · [ ] 大小 · Shift-[ ] 硬度 · 1–0 不透明度 · Escape 取消 · 空格平移" : session.tool == .blur ? (session.blurMode == .blur ? "拖动模糊" : session.blurMode == .smudge ? "拖动涂抹" : "拖动液化像素") + " · [ ] 大小 · Shift-[ ] 硬度 · 1–0 强度 · 空格平移" : session.tool == .cloneStamp ? "Option 点击设置取样源 · 拖动仿制 · [ ] 大小 · Shift-[ ] 硬度 · 1–0 不透明度 · 空格平移" : session.tool == .spotHealing ? "在瑕疵上拖动修复 · [ ] 大小 · Shift-[ ] 硬度 · Escape 取消 · 空格平移" : session.tool == .type ? "拖动创建文本框 · 点击文本编辑 · 拖动手柄改大小 · ⌘Return 完成 · Escape 取消" : session.tool == .shape ? "Drag to draw a shape on a new layer · Shift \(session.shapeKind == .line ? "45°" : session.shapeKind == .rectangle ? "正方形" : "圆形") · Option from center · Shift-U or Tab for the next shape · Escape cancel · Space to pan" : session.tool == .gradient ? "拖动绘制 · 拖动端点调整 · Shift 45° · 1–0 不透明度 · 回车应用 · Escape 取消" : session.tool == .crop ? "拖动裁剪 · 回车应用 · Escape 取消 · 空格平移" : session.tool == .move ? "拖动移动 · 手柄调整大小 · 圆形手柄旋转 · 1–0 图层不透明度 · 空格平移" : session.tool == .hand ? "拖动平移 · 双指缩放" : session.tool == .idle ? "未选择工具 · 按工具快捷键选择 · 空格平移" : "点击放大 · Option 点击缩小 · 左右拖动平滑缩放 · 空格平移")
            }
        }
