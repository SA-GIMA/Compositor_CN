    ...
    var body: some View {
        ...
        .toolbar {
            ...
            ToolbarItemGroup(placement: .primaryAction) {
                Button("适合") { session.fit() }.help("适合画布到窗口（⌘0）")
    ...
    private var statusBar: some View {
        HStack(spacing: 16) {
            ...
            if session.showsBusy {
                ...
                Text(session.tool == .marquee ? (session.marqueeKind == .ellipse ? "拖动绘制椭圆 · Shift 添加 · Option 减去 · 拖动中再按 Shift 为正圆 · 在内部拖动移动 · Delete 清除 · ⌘D 取消选择" : "拖动绘制矩形 · Shift 添加 · Option 减去 · 拖动中再按 Shift 为正方形 · 在内部拖动移动 · ⌘ 拖动移动像素 · Delete 清除 · ⌘D 取消选择") : session.tool == .wand ? "点击选择相近颜色 · Shift 添加 · Option 减去 · 在内部拖动移动 · ⌘ 拖动移动像素 · Delete 清除 · ⌘D 取消选择" : session.tool == .lasso ? (session.lassoKind == .freehand ? "拖动建立选区 · 在内部拖动移动 · Shift 添加 · Option 减去 · Delete 清除 · ⌥⌫/⌘⌫ 填充 · ⌘D 取消选择" : "点击添加角点 · 点击起点、双击或回车闭合 · Delete 删除角点 · Escape 取消") : session.tool == .brush ? (session.brushMode == .erase ? "拖动擦除" : "拖动绘制") + " · [ ] 大小 · Shift-[ ] 硬度 · 1–0 不透明度 · Escape 取消 · 空格平移" : session.tool == .blur ? (session.blurMode == .blur ? "拖动柔化" : session.blurMode == .smudge ? "拖动涂抹" : "拖动推动像素") + " · [ ] 大小 · Shift-[ ] 硬度 · 1–0 强度 · 空格平移" : session.tool == .cloneStamp ? "Option 点击取样源 · 拖动仿制 · [ ] 大小 · Shift-[ ] 硬度 · 1–0 不透明度 · 空格平移" : session.tool == .spotHealing ? "在瑕疵上拖动修复 · [ ] 大小 · Shift-[ ] 硬度 · Escape 取消 · 空格平移" : session.tool == .shape ? "拖动在新图层上绘制形状 · Shift \(session.shapeKind == .rectangle ? "正方形" : "圆形") · Option 从中心绘制 · Shift-U 切换\(session.shapeKind == .rectangle ? "椭圆" : "矩形") · Escape 取消 · 空格平移" : session.tool == .gradient ? "拖动绘制 · 拖动端点调整 · Shift 45° · 1–0 不透明度 · 回车应用 · Escape 取消" : session.tool == .crop ? "拖动裁剪 · 回车应用 · Escape 取消 · 空格平移" : session.tool == .move ? "拖动移动 · 控制点缩放 · 圆圈旋转 · 1–0 图层不透明度 · 空格平移" : session.tool == .hand ? "拖动平移 · 双指缩放" : session.tool == .idle ? "未选择工具 · 按工具快捷键选择 · 空格平移" : "点击放大 · Option 点击缩小 · 左右拖动平滑缩放 · 空格平移")
            }
        }
