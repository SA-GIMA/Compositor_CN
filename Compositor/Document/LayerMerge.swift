...
extension EditorSession {
    ...
    private func mergePlan() -> (ids: [UUID], removed: Set<UUID>, name: String, parent: UUID?, anchor: UUID, action: String)? {
        ...
        if selectedLayerIDs.count > 1 {
            ...
            return (ordered.map(\.id), picked, top.name, top.parentID, top.id, "合并图层")
        }
        if active.isGroup {
            ...
            return (ids, Set(ids), active.name, active.parentID, active.id, "合并组")
        }
        ...
        return ([below.id, active.id], [below.id, active.id], below.name, active.parentID, active.id, "向下合并")
    }

    var canMergeLayers: Bool { mergePlan() != nil }
    var mergeTitle: String { mergePlan()?.action ?? "向下合并" }
