...
private final class LayerCell: NSTableCellView, NSTextFieldDelegate {
    ...
    override init(frame frameRect: NSRect) {
        ...
        thumbnail.toolTip = "选择图层；Cmd 点击选择其像素（Cmd-Shift 添加，Cmd-Option 减去）"
        ...
        let menu = NSMenu()
        for (title, action) in [("重命名…", #selector(rename)), ("隐藏/显示图层", #selector(toggleVisibility)),
                                ("添加白色蒙版", #selector(addWhiteMask)), ("添加黑色蒙版", #selector(addBlackMask)),
                                ("启用/停用蒙版", #selector(toggleMask)), ("删除蒙版", #selector(deleteMask)), ("释放剪贴蒙版", #selector(removeLiveMask)),
                                ("从文件夹中移出", #selector(moveOut)), ("删除图层/文件夹", #selector(deleteLayer))] {
    ...
    func configure(_ layer: ImageLayer, enabled: Bool, session: EditorSession, depth: Int, visible: Bool) {
        ...
        thumbnail.toolTip = "选择图像像素"
        maskThumbnail.toolTip = "选择图层蒙版；Shift 点击启用/停用；Cmd 点击选择其黑色区域（Cmd-Shift 添加，Cmd-Option 减去）"
        thumbnail.setAccessibilityLabel("选择图像：\(layer.name)")
        ...
        dimensions.stringValue = layer.adjustment != nil ? "调整图层 · 双击编辑" : layer.isGroup ? "文件夹" : "\(Int(layer.size.width.rounded())) × \(Int(layer.size.height.rounded())) px"
        if let source = layer.maskSourceID {
            let sourceName = session.document?.layers.first(where: { $0.id == source })?.name ?? "源丢失"
    ...
    func updateTarget() {
        if let layer = session?.document?.layers.first(where: { $0.id == layerID }),
           ...
           let source = session?.document?.layers.first(where: { $0.id == sourceID }) {
            ...
            dimensions.toolTip = "基于 \(source.name) 的剪贴蒙版。Option 点击其行底部可释放。"
        }
